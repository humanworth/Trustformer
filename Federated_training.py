...
def evaluate_workers(workers, store=True):
    print("start evaluating workers...")
    results = []
    for worker in workers:
        results, loss, metrics = worker.evaluate_model()
        if store is True:
            file_path = config['results']+f'/evaluation_{worker.name}.pkl'
            with open(file_path, 'wb') as file:
                pickle.dump({'worker': worker.name,'results': results, 'loss': loss, 'metrics': metrics}, file)
        results.append({'worker': worker.name,'results': results, 'loss': loss, 'metrics': metrics})
    return results
